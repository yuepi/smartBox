...
import ExportNotification from "#/components/ExportNotification/index.vue";
...
const menus = computed(() => {
  ...
  if (merchantList.value.length > 0) {
    menuItems.push({
      ...
      text: `切换商户${
        currentMerchantName.value ? ` (${currentMerchantName.value})` : ""
      }`,
    });
  }
  ...
  >
    <div class="merchant-dialog-list">
      <div
        ...
        :class="{
          active: merchant.merchantId === userStore.userInfo?.merchantId,
        }"
      ...
      >
        ...
        <el-tag
          v-if="merchant.merchantId === userStore.userInfo?.merchantId"
          type="success"
          size="small"
        >
          当前
