...
import { addPlatMenuApi, clearMenuCacheApi, deletePlatMenuApi, editPlatMenuApi, getPlatMenuDetailApi, getPlatMenuListApi, refreshMenuCacheApi } from '#/api/system/menu';
...
const tableColumns = computed(() => {
  ...
  const mappedColumns = cols.map((col) => ({
    ...
    cellRenderer: ({ rowData, cells }: { cells: any[]; rowData: Menu; }) => {
      switch (col.key) {
        case 'code': {
          return <span class="text-sm text-gray-500">{rowData.code || '-'}</span>;
        }
        case 'component': {
          return <span class="truncate text-sm text-gray-500">{rowData.component || '-'}</span>;
        }
        case 'menuName': {
          return (
        ...
        case 'path': {
          return <span class="truncate text-sm text-gray-500">{rowData.path || '-'}</span>;
        }
        case 'platformType': {
          return (
