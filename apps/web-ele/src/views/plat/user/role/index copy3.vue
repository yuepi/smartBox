...
const handleTabClick = async (tab: any) => {
  ...
  if (tab.paneName === "dataScope" && deptTreeData.value.length === 0) {
    await loadDeptTree();
    // 回显已选中的部门
    if (formData.value.deptIds?.length && deptTreeRef.value) {
      await nextTick();
      deptTreeRef.value.setCheckedKeys(formData.value.deptIds);
    }
  }
};
...
async function handleSubmit() {
  ...
  // 获取选中的部门权限ID（自定义数据权限时）
  if (formData.value.scope === 2 && deptTreeRef.value) {
...
<template>
  <Page auto-content-height>
    <h1 class="text-2xl font-bold">平台角色管理</h1>
    <div class="p-4">
      <!-- 查询表单 -->
      <el-card shadow="never" class="mb-4">
        <el-form :inline="true" :model="queryParams">
          ...
          <el-form-item>
            <el-button type="primary" :icon="Search" @click="handleQuery" v-access:code="['plat:role:page']">
              查询
            </el-button>
            <el-button :icon="Refresh" @click="resetQuery" v-access:code="['plat:role:page']">重置</el-button>
            <el-button type="primary" plain :icon="Plus" @click="handleAdd" v-access:code="['plat:role:add']">
              新增
            </el-button>
            <el-button
              type="danger"
