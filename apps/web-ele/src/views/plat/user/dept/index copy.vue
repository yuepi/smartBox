...
import { addPlatDeptApi, deletePlatDeptApi, editPlatDeptApi, getPlatDeptDetailApi, getPlatDeptListApi } from '#/api/system/dept';
        ...
        case 'deptName': {
          return (
            <div class="flex w-full items-center gap-1 overflow-hidden">
              {rowData.deptType === 0 && <HomeFilled class="text-primary h-4 w-4 shrink-0" />}
              {rowData.deptType === 1 && <OfficeBuilding class="text-success h-4 w-4 shrink-0" />}
              {rowData.deptType === 2 && <Folder class="text-info h-4 w-4 shrink-0" />}
