...
const calcAfterWeight = computed(() => {
  ...
  return form.operateType === 0
    ? Number((origin + delta).toFixed(2))
    : Number(Math.max(0, origin - delta).toFixed(2));
});
...
function handleAmountInput(amount = 0) {
  form.amount = amount;
  form.weight =
    orderInfo.unitPrice > 0
      ? Number((amount / orderInfo.unitPrice).toFixed(2))
      : 0;
}
...
<template>
  ...
  >
    <el-form :model="form" label-width="110px" class="custom-form">
      ...
      <!-- 操作重量 -->
      <el-form-item
        :label="`${form.operateType === 0 ? '补重' : '扣重'}重量`"
        required
      >
        <el-input-number
          v-model="form.weight"
          :min="0"
          :max="form.operateType === 1 ? orderInfo.weight : 999"
          :precision="2"
          :step="0.1"
          placeholder="请输入重量"
          class="full-width"
      ...
      <!-- 变动金额 (支持双向联动) -->
      <el-form-item
        :label="`预估${form.operateType === 0 ? '补发' : '扣除'}金额`"
      >
        <el-input-number
          v-model="form.amount"
          :min="0"
      ...
      <div class="calc-preview-card">
        <div class="preview-row">
          ...
          <span class="value">
            {{ orderInfo.weight }} {{ form.operateType === 0 ? '+' : '-' }}
            {{ form.weight }} =
        ...
        <div class="preview-row">
          ...
          <span class="value">
            ¥ {{ orderInfo.realAmount.toFixed(2) }}
            {{ form.operateType === 0 ? '+' : '-' }} ¥
            {{ form.amount.toFixed(2) }} =
            <strong class="highlight-text"
              >¥ {{ calcAfterAmount.toFixed(2) }}</strong
            >
          </span>
        </div>
      </div>
      ...
      <div class="bottom-info-bar">
        <span class="info-item">
          ...
          原投递量
          <strong class="text-gray-700 ml-0.5"
            >{{ orderInfo.weight }} kg</strong
          >
        </span>
