...
async function handleItemSubmit() {
  ...
  try {
    const api = itemFormData.value.sortItemId
      ? editSortItemApi
      : addSortItemApi;
...
async function handleDeleteItem(row: SortItem) {
  try {
    await ElMessageBox.confirm(`确定要删除这条分拣明细吗？`, '提示', {
      type: 'warning',
    });
  ...
  <Page auto-content-height>
      ...
      <el-card shadow="never" class="mb-4">
        <el-form :inline="true" :model="queryParams">
          ...
          <el-form-item label="分拣状态">
            ...
            >
              <el-option
                v-for="item in sortStatusOptions"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              />
    ...
    >
      <el-descriptions :column="2" border v-if="detailData">
        <el-descriptions-item label="分拣单号" :span="2">{{
          detailData.sortNo
        }}</el-descriptions-item>
        <el-descriptions-item label="关联清运任务">{{
          detailData.cleanTaskId || '-'
        }}</el-descriptions-item>
        ...
        <el-descriptions-item label="包袋ID">{{
          detailData.deviceBagId || '-'
        }}</el-descriptions-item>
        <el-descriptions-item label="总重量"
          >{{
            detailData.totalWeight?.toFixed(2) || 0
          }}
        ...
        <el-descriptions-item label="分拣人员">{{
          detailData.sortUserName || '-'
        }}</el-descriptions-item>
    ...
    <el-dialog
      v-model="itemVisible"
      :title="`分拣明细 - ${currentSortTask?.sortNo}`"
      width="700px"
      append-to-body
      @close="itemList = []"
    >
    ...
    >
      <el-form :model="itemFormData" label-width="80px">
        <el-form-item label="分类类型" required>
          <el-select
            v-model="itemFormData.packageType"
            placeholder="请选择"
            style="width: 100%"
          >
            <el-option
        ...
        <el-form-item label="分类名称">
          <el-input
            v-model="itemFormData.packageName"
            placeholder="请输入分类名称（可选）"
          />
        </el-form-item>
        <el-form-item label="重量(kg)" required>
          <el-input-number
            v-model="itemFormData.weight"
            :min="0"
            :precision="2"
            :step="0.1"
            style="width: 100%"
