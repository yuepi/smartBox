...
async function handleDelete(row?: CleanTask) {
  ...
  try {
    await ElMessageBox.confirm(`确定要删除选中的 ${ids.length} 条任务吗？`, '提示', { type: 'warning' });
      ...
      <el-card shadow="never" class="mb-4">
        <el-form :inline="true" :model="queryParams">
          <el-form-item label="任务单号">
            <el-input
v-model="queryParams.taskNo" placeholder="请输入任务单号" clearable style="width: 180px"
              @keyup.enter="handleQuery"
/>
          </el-form-item>
          <el-form-item label="所属小区">
            <el-select v-model="queryParams.deptId" placeholder="请选择小区" clearable filterable style="width: 160px">
...
/>
    ...
    <el-dialog v-model="formVisible" :title="formTitle" width="550px" append-to-body>
      <el-form :model="formData" label-width="100px">
        <el-form-item label="所属小区">
          <el-select v-model="formData.deptId" placeholder="请选择小区" clearable filterable style="width: 100%">
            <el-option v-for="item in deptOptions" :key="item.deptId" :label="item.deptName" :value="item.deptId" />
          </el-select>
        </el-form-item>
        <el-form-item label="选择设备" required>
          <el-select
v-model="formData.deviceId" placeholder="请选择设备" clearable filterable style="width: 100%"
...
/>
        ...
        <el-form-item label="选择仓口">
          <el-select
v-model="formData.hatchId" placeholder="请先选择设备" clearable style="width: 100%"
            :disabled="!formData.deviceId"
>
            <el-option
v-for="item in hatchOptions" :key="item.deviceHatchId"
              :label="`${item.hatchNo}号仓 - ${item.hatchName}`" :value="item.deviceHatchId"
/>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="计划时间">
          <el-date-picker v-model="formData.planTime" type="datetime" placeholder="选择计划清运时间" style="width: 100%" />
        </el-form-item>
    ...
    <!-- 详情弹窗 -->
    <el-dialog v-model="detailVisible" title="清运任务详情" width="600px" append-to-body>
      <el-descriptions :column="2" border v-if="detailData">
        <el-descriptions-item label="任务单号" :span="2">{{ detailData.taskNo }}</el-descriptions-item>
        <el-descriptions-item label="所属小区">{{ detailData.deptId || '-' }}</el-descriptions-item>
...
{{ detailData.hatchNo ? `${detailData.hatchNo}号仓` : '-'
        ...
        <el-descriptions-item label="满仓重量">{{ detailData.fullWeight?.toFixed(2) || 0 }} kg</el-descriptions-item>
        <el-descriptions-item label="清运人员">{{ detailData.cleanUserName || '-' }}</el-descriptions-item>
        <el-descriptions-item label="计划时间">{{ detailData.planTime || '-' }}</el-descriptions-item>
