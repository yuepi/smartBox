...
import { addDevicePackageApi, editDevicePackageApi } from '#/api/device/devicePackage';
...
async function handleSubmit() {
  ...
  loading.value = true;
  try {
    const api = formData.value.devicePackageId ? editDevicePackageApi : addDevicePackageApi;
  ...
  <el-dialog v-model="visible" :title="title" width="500px" append-to-body>
    <el-form :model="formData" label-width="100px">
      <el-form-item label="套餐名称" required>
        <el-input v-model="formData.packageName" placeholder="请输入套餐名称，如：混合回收套餐" />
      </el-form-item>
      <el-form-item label="计费类型">
        <el-radio-group v-model="formData.packageType">
          <el-radio v-for="item in package_type" :key="item.value" :value="item.value">
            {{ item.label }}
          </el-radio>
        </el-radio-group>
      </el-form-item>
      <el-form-item label="回收单价" required>
        <el-input-number v-model="formData.unitPrice" :min="0" :precision="2" :step="0.1" style="width: 100%" />
