  ...
  <el-dialog v-model="visible" title="出库单详情" width="800px" append-to-body>
    <div v-loading="loading">
      ...
      <el-descriptions :column="2" border v-if="detailData">
        ...
        <el-descriptions-item label="总重量">{{ detailData.totalOutWeight?.toFixed(2) }} kg</el-descriptions-item>
        <el-descriptions-item label="销售总金额">¥ {{ detailData.totalSaleAmount?.toFixed(2) }}</el-descriptions-item>
        <el-descriptions-item label="回收商">{{ detailData.customerId || '-' }}</el-descriptions-item>
      ...
      <div class="mt-4">
        ...
        <el-table :data="itemList" border stripe style="width: 100%">
          ...
          <el-table-column prop="outWeight" label="出库重量(kg)" width="140" align="center">
            <template #default="{ row }">
              {{ row.outWeight?.toFixed(2) || 0 }}
            </template>
          </el-table-column>
          <el-table-column prop="saleUnitPrice" label="销售单价(元/kg)" width="150" align="center">
            <template #default="{ row }">
              {{ row.saleUnitPrice?.toFixed(2) || 0 }}
            </template>
          </el-table-column>
          <el-table-column prop="saleAmount" label="销售金额(元)" width="150" align="center">
            <template #default="{ row }">
