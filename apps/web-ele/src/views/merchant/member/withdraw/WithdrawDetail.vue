...
<template>
  <el-dialog v-model="visible" title="提现详情" width="700px" append-to-body>
    <el-descriptions :column="2" border v-if="detailData">
      ...
      <el-descriptions-item label="申请金额">{{ formatAmount(detailData.applyAmount) }}</el-descriptions-item>
      <el-descriptions-item label="平台服务费">{{ formatAmount(detailData.platformFee) }}</el-descriptions-item>
      <el-descriptions-item label="实际到账金额">
        <span class="font-bold text-success">{{ formatAmount(detailData.realWithdrawAmount) }}</span>
      </el-descriptions-item>
      <el-descriptions-item label="审核模式">
        <el-tag :type="detailData.auditMode === 0 ? 'warning' : 'success'" size="small">
      ...
      <el-descriptions-item v-if="detailData.auditReason" label="驳回原因" :span="2">
      ...
      <el-descriptions-item label="支付请求时间">{{ detailData.payRequestTime || '-' }}</el-descriptions-item>
    </el-descriptions>
    <template #footer>
