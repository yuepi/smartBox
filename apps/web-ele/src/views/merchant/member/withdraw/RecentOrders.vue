...
<template>
  <el-dialog v-model="visible" :title="`近期订单 - ${memberName}`" width="900px" append-to-body @close="orders = []">
    <div v-loading="loading">
      <el-table :data="orders" border stripe style="width: 100%">
        <el-table-column prop="orderNo" label="订单编号" min-width="200" align="center" show-overflow-tooltip />
        <el-table-column prop="weight" label="投递重量" width="110" align="center">
          <template #default="{ row }">{{ row.weight?.toFixed(2) || 0 }} kg</template>
        </el-table-column>
        <el-table-column prop="realAmount" label="实际金额" width="120" align="center">
