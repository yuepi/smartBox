...
// 表单响应式数据定义
const formData = reactive<Partial<HomeItem>>({
  ...
  try {
    ...
    categoryTreeOptions.value =
      Array.isArray(list) && list.length > 0
        ? (list[0]?.children ? list : handleListToTree(list, 0))
        : [];
  } catch {
    categoryTreeOptions.value = [];
  }
...
function generateSkuCombos() {
  ...
  (formData.skuComboList || []).forEach((item) => {
    const key = Array.isArray(item.dimValueIds) ? item.dimValueIds.join('/') : item.comboName;
  ...
  if (isEdit.value && row?.homeItemId) {
    ...
    try {
      ...
      if (typeof detail.imageUrls === 'string') {
        imageFileList.value = (detail.imageUrls as string).split(',').map((s) => s.trim()).filter(Boolean);
  ...
  >
    ...
    >
      ...
      <el-form-item label="服务名称" prop="itemName">
        <el-input v-model="formData.itemName" placeholder="例如：柜式空调移机、挂机深度清洗" />
      </el-form-item>
      ...
      <div class="mb-2 mt-6 flex items-center justify-between border-b pb-1">
        <div>
          ...
          <span class="ml-2 text-xs text-gray-400">比如：匹数、楼层、型号等属性</span>
        </div>
        <el-button type="primary" plain size="small" icon="Plus" @click="addSkuDim">
          添加规格项
        </el-button>
      </div>
      ...
      >
        <div class="flex items-center gap-3">
          ...
          <el-button type="danger" link size="small" @click="removeSkuDim(index)">删除此规格</el-button>
        </div>
        ...
        <div class="mt-2 flex flex-wrap items-center gap-2">
          ...
          <div class="flex items-center gap-1">
            ...
            />
            <el-button size="small" @click="addDimValue(index)">回车添加</el-button>
          </div>
        </div>
      </div>

      <!-- 组合价格表 -->
      <div v-if="formData.skuComboList && formData.skuComboList.length > 0" class="mt-4">
        <div class="mb-1 text-xs font-bold text-gray-600">各规格组合价格明细：</div>
        <el-table :data="formData.skuComboList" border size="small">
          ...
          <el-table-column label="实付价/到手价 (元)" width="150">
            <template #default="{ row }">
              <el-input-number v-model="row.price" :min="0" :precision="2" size="small" controls-position="right" class="!w-full" />
            </template>
          </el-table-column>
          <el-table-column label="原价/划线价 (元)" width="150">
            <template #default="{ row }">
              <el-input-number v-model="row.marketPrice" :min="0" :precision="2" size="small" controls-position="right" class="!w-full" />
            </template>
          </el-table-column>
      ...
      <div class="mb-2 mt-6 flex items-center justify-between border-b pb-1">
        <div>
          ...
          <span class="ml-2 text-xs text-gray-400">比如：加急费、高空作业费、打孔费等选填加价项</span>
        </div>
        <el-button type="primary" plain size="small" icon="Plus" @click="addOptionGroup">
          添加加价组
        </el-button>
      </div>
      ...
      >
        <div class="flex items-center gap-3">
          ...
          <el-input v-model="group.optionName" placeholder="如：墙体打孔服务" class="!w-44" size="small" />
          ...
          <el-checkbox v-model="group.required" :true-value="1" :false-value="0" size="small">必选</el-checkbox>
          <el-button type="primary" link size="small" @click="addOptionValue(gIdx)">添加选项</el-button>
          <el-button type="danger" link size="small" @click="removeOptionGroup(gIdx)">删除组</el-button>
        </div>

        <div v-for="(val, vIdx) in group.valueList" :key="vIdx" class="mt-2 flex items-center gap-2 pl-4">
          <span class="text-xs text-gray-400">选项：</span>
          <el-input v-model="val.valueName" placeholder="如：混凝土打孔" class="!w-40" size="small" />
          
          <span class="text-xs text-gray-500">加价金额：</span>
          <el-input-number v-model="val.priceDelta" :precision="2" size="small" controls-position="right" class="!w-32" />
          <span class="text-xs text-gray-400">元</span>
          
          <el-button type="danger" icon="Delete" circle size="small" link @click="removeOptionValue(gIdx, vIdx)" />
        </div>
      </div>
    </el-form>
