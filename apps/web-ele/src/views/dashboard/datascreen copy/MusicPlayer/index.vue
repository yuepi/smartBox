...
// 当前正在播放的歌曲配置对象
const currentSong = computed<SongItem | undefined>(
  () => props.songs[currentIndex.value],
);
...
const handleSeek = (e: Event) => {
  ...
  audio.currentTime =
    song && song.loopEnd !== undefined && targetTime > song.loopEnd
      ? song.loopStart || 0
      : targetTime;
};
...
<template>
  <div
    ...
    :class="
      isExpanded
        ? 'max-w-[400px] h-11 px-3 gap-3'
        : 'w-11 h-11 justify-center p-0 gap-0'
    "
  >
    <div
    ...
    >
      <button
        class="hover:text-cyan-400 text-sm transition-colors cursor-pointer"
        title="上一曲"
        @click="prevTrack"
      >
        ⏮
      ...
      <div class="flex flex-col max-w-[80px]">
        <span
          class="text-[10px] truncate text-slate-300"
