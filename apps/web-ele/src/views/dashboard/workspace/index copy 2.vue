...
async function fetchOverview() {
  ...
  try {
    const params = overviewDateRange.value
      ? {
          startTime: overviewDateRange.value[0],
          endTime: overviewDateRange.value[1],
        }
      : undefined;
...
// 排行榜
const rankingType = ref<'memberCount' | 'memberWeight' | 'merchantWeight'>(
  'memberWeight',
);
...
function renderRankingChart(
  list: { name: string; phone: string; value: number }[],
) {
...
// ==================== 5. 设备状态 ====================
const deviceData = ref<DeviceStatusData>({
  deviceTotal: 0,
  onlineCount: 0,
  offlineCount: 0,
});
...
<template>
  ...
  >
    ...
    >
      ...
      >
        <div class="text-lg font-bold mb-3 z-10 relative">
          您好，欢迎智能回收箱系统！
        </div>
        <div
          class="bg-white/95 text-gray-800 rounded-lg p-3 text-xs space-y-2 shadow-inner relative z-10"
        >
          <div class="flex items-center gap-2.5">
            <el-avatar
              :size="36"
              :src="
                userStore.userInfo?.user?.avatar ||
            ...
            <div>
              <div class="font-bold text-sm">
                {{ userStore.userInfo?.user?.nickName || '超级管理员' }}
              </div>
          ...
          <div class="text-gray-500 pt-1.5 border-t border-gray-100">
            ...
            <span class="text-primary-600 font-medium">{{
              currentMerchantInfo?.merchantName || 'huixiaofen'
            }}</span>
          </div>
        ...
        <div
          class="absolute -right-8 -bottom-8 w-28 h-28 bg-white/10 rounded-full blur-sm"
        ></div>
      </div>
      ...
      >
        <div class="grid grid-cols-3 gap-2 text-center">
          <div class="bg-gray-50 dark:bg-zinc-700/40 p-2 rounded-lg">
            ...
            <div
              class="text-lg font-bold text-gray-800 dark:text-gray-100 mt-0.5"
            >
          ...
          <div class="bg-blue-50/60 dark:bg-blue-950/30 p-2 rounded-lg">
            ...
            <div
              class="text-lg font-bold text-blue-600 dark:text-blue-400 mt-0.5"
            >
      ...
      >
        ...
        <div class="grid grid-cols-3 xl:grid-cols-6 gap-3">
          ...
          >
            ...
            <div
              class="text-lg font-bold text-gray-800 dark:text-gray-100 my-0.5"
            >
              {{ card.today }}
            </div>
            <div
