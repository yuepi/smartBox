...
async function onSearch() {
  receItems.value = await requestClient.get(`/gen/org/main?type=${state.p_org_type}&name=${state.receSearch}`);
  for (const rItem of receItems.value) {
    let flag = false;
    for (const sItem of seldItems.value) {
      if (sItem.id === rItem.id) {
...
async function roleNodeClick(node: any) {
  if (node && node.id) {
    roleItems.value = await requestClient.get(`/gen/org/role/list?treid=${node.id}`);
    for (const rItem of roleItems.value) {
      for (const sItem of seldItems.value) {
        if (sItem.id === rItem.id) {
          rItem.checked = true;
